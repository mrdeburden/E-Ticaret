...
<head>
    ...
    <style>
        ...
        .edit-btn { background: #3498db; color: white; padding: 10px 15px; border-radius: 6px; text-decoration: none; font-size: 14px; display: inline-block; }
...
<body>
    ...
    <div class="container">
        <div style="display:flex; justify-content:space-between; align-items:center;">
            <h2 style="color:#333; border-bottom:2px solid #ddd; padding-bottom:10px;"><i class="fas fa-edit" style="color:#f39c12;"></i> İlanlarım</h2>
            <a href="urun_ekle.php" class="add-new-btn">+ Yeni Ürün Ekle</a>
        </div>

        <?php if (isset($_GET['durum'])): ?>
            <?php if ($_GET['durum'] == 'guncellendi'): ?>
                <div style="padding:10px; background:#d4edda; color:#155724; border-radius:5px; margin-bottom:15px;">Ürün başarıyla güncellendi.</div>
            <?php elseif ($_GET['durum'] == 'silindi'): ?>
                <div style="padding:10px; background:#f8d7da; color:#721c24; border-radius:5px; margin-bottom:15px;">Ürün silindi.</div>
            <?php endif; ?>
        <?php endif; ?>
                    ...
                    <div class="product-brief">
                        ...
                        <div>
                            ...
                            <div style="font-size:14px; color:#6A0DAD; font-weight:bold; margin-top:5px;">
                                Stok: <?php echo $urun['stock_quantity']; ?> | <?php echo number_format($urun['price'], 2, ',', '.'); ?> ₺
                            </div>
                        </div>
                    </div>

                    <div class="btn-group">
                        <a href="urun_duzenle.php?id=<?php echo $urun['product_id']; ?>" class="edit-btn">
                            <i class="fas fa-pen"></i> Düzenle
                        </a>

                        <form action="ilan_islem.php" method="POST" onsubmit="return confirm('Silmek istediğine emin misin?');" style="margin:0;">
                            <input type="hidden" name="product_id" value="<?php echo $urun['product_id']; ?>">
                            <button type="submit" name="urun_sil" class="delete-btn"><i class="fas fa-trash-alt"></i></button>
                        </form>
                    </div>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
            <p style="text-align:center; padding:50px; color:#888;">Henüz ilanınız yok.</p>
        <?php endif; ?>
    </div>
</body>
</html>